...
class Cart extends Db{
    ...
    public function checkCart( $id_user , $id_product){
        $sql = self::$connection->prepare("SELECT * FROM `cart` WHERE `id_user` = ? AND `id_product` = ?");
    ...
    public function getCart1($id_user,$id_product){
        $sql = self::$connection->prepare("SELECT * FROM `cart` WHERE `id_user` = ? AND `id_product` = ?");
